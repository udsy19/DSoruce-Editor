...
const REPO_SKIP = new Set([
  ...
  // `dist-server` is `deploy/`'s build output — the esbuild bundle of the Node
  // service — and belongs to the same category as `dist` and `target` beside it.
  // It is gitignored and regenerated by `npm run bundle`, which `deploy.sh` and
  // the Dockerfile both run, so it appears in a working tree the moment anyone
  // prepares a deploy. Without this the census reads the BUNDLE, sees the
  // assertion idioms of every source esbuild inlined into it, and reds — naming
  // a file that is not a check and cannot be wired into a board.
  'dist-server',
