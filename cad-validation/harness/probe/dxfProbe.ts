...
export function parseDrawing(dxfText: string): Drawing {
  ...
  // PROBE: capture pre-cluster state (diagnostic copy only)
  const preBounds: [number, number, number, number] = [Infinity, Infinity, -Infinity, -Infinity]
  accumulateBounds(entities, preBounds)
  for (const f of furniture) accumulateBounds(f.entities, preBounds)
  const preCount = { entities: entities.length, furniture: furniture.length }
  // Drop mirrored/xref duplicate geometry that sits far from the main plan.
  const kept = keepDominantCluster(entities, furniture)
  ;(globalThis as any).__probe = { preBounds, preCount, postCount: { entities: kept.entities.length, furniture: kept.furniture.length } }
