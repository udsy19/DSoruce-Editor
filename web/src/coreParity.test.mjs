...
// --- 1c. GROUND: RETIRED — superseded by the generated domain -----------------
//
// This checked `types/doc.ts`'s `GROUND_ZONES` literal against the core. Both
// halves are gone: the Rust side is now generated by `zone_domain!` (one
// authoring point) and the TS side is `zoneDomain.generated.ts`, produced by
// EVALUATING the compiled core. `GROUND_ZONES` is a re-export, so there is no
// literal left to parse and nothing for a mirror check to mirror.
//
// **Retired rather than rewritten**, because rewriting it would leave two
// checks certifying one property — and R20 says witnesses sharing a mechanism
// are one witness. `node scripts/gen-zone-domain.mjs --check` is strictly
// stronger: it compares the WHOLE domain (types and ground set) against what
// the core answers, it is a battery step, and staleness is a build failure.
// This row would have added nothing but a second place to be wrong.
