...
export function ProgramStep({
  ...
  return (
    <div className="program-step" data-testid="program-step">
      ...
      <div className={`program-body${drawing ? ' has-anchors' : ''}`}>
        ...
        {/* Anchor pins (workflow.md §3.5): pick a room type, click the plan to
            ...
            It used to sit BELOW the body as a full-width band. That put a 340px
            preview under a 627px fold — clipped by 252px at the panel edge — and
            spread a plan that is taller than it is wide across 1278px, so the
            drawing rendered 232px wide inside 1044px of empty margin (measured,
            1491x812). As a column of the body it fills the work area's height
            and the plan fits to the tall axis instead of the wide one. */}
        {drawing && (
          <section className="program-anchors" data-testid="program-anchors">
            ...
            {anchorTool && (
              <p className="space-tool-hint" data-testid="program-anchor-hint">
                Click the plan to drop a <strong>{anchorKindLabel(anchorKind)}</strong> pin. Esc to
                stop.
              </p>
            )}
